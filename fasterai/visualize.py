...
class ModelImageVisualizer():
    ...
    def plot_transformed_image(self, path:str, figsize:(int,int)=(20,20), render_factor:int=None)->ndarray:
        path = Path(path)
        result = self.get_transformed_image_ndarray(path, render_factor)
        ...
        if self.results_dir is not None:
            self.save_result_image(path, result)

    def get_transformed_image_as_pil(self, path:str, render_factor:int=None)->Image:
        path = Path(path)
        array = self.get_transformed_image_ndarray(path, render_factor)
        return misc.toimage(array)

    def save_result_image(self, source_path:Path, result:ndarray):
        result_path = self.results_dir/source_path.name
        misc.imsave(result_path, np.clip(result,0,1))

    def get_transformed_image_ndarray(self, path:Path, render_factor:int=None):
